    ...
    for j in range(1):
        for i in range(1200):
            ...
            print_two_with_colorbar(
                theta, phi, f"{i}", folder=f"theta_star/animation/{n}"
            )
...
def post_prod():
    with open(f'theta_star/theta_avg1.txt') as f:
        Y = list(abs(float(xx)) for xx in f.read().split())

    X = list(eps * 1 / len(Y) for eps in range(len(Y)))
    pyplot.plot(X, Y, '-')
    with open(f'theta_star/theta_avg2.txt') as f:
        Y = list(abs(float(xx)) for xx in f.read().split())
    pyplot.plot(X, Y, '--')
    pyplot.ylim(-0, 1.1)
    pyplot.xlabel(r"$t$")
    ...
    green_legend = lines.Line2D(
        [], [], color='grey', linestyle='-', label=r'$\theta_* = 0.2$'
    )
    blue_legend = lines.Line2D(
        [], [], color='black', linestyle='--', label=r"$\theta_* = 0.6$"
    )
    plt.legend(handles=[green_legend, blue_legend], prop={'size': 10})
    pyplot.savefig(f'theta_star/theta_dyn_star.eps', )
    pyplot.savefig(f'theta_star/theta_dyn_star.png', )
