...
import numpy as np
def surf_draw(name):
    with open(f'scratch/{name}_ans_gamma_dynamic.txt') as file:
    ...
    # fig.colorbar(p)
    plt.savefig(f'{name}_dynamic.eps', bbox_inches="tight")
    plt.savefig(f'{name}_dynamic.png', bbox_inches="tight")
