...
При указанных параметрах для первого эксперимента положим тестовое значение $u$ (рис. \ref{control}\subref{fig1:exp1}):
\begin{equation}
u(x)=   
\begin{cases}
...
\end{cases} 
\end{equation}
и для второго эксперимена (рис. \ref{control}\subref{fig1:exp2}):
\begin{equation}
  \label{test_function_1}
  u(x)=0.49x+0.01. \; 
\end{equation}

Вычислим решение прямой задачи \eqref{initial}--\eqref{initial_boundary} для этих случаев. Полученное температурное поле обозначим за $\theta_0$. Далее, применяя предложенный алгоритм находим квазирешение обратной задачи \eqref{initial}--\eqref{theta_gamma}. Эффективность алгоритма, а также значение $u_0$ в первом и втором случаях также приведены на рис. \ref{control}. На рис. \ref{cost} показана динамика функционала качества по итерациям.
...
{
	\label{fig1:exp1}
	\includegraphics[width=.51\linewidth]{first/control_initial_optimal_final.png}
}
\subfloat[Второй эксперимент]
{
	\label{fig1:exp2}
	\includegraphics[width=.51\linewidth]{second/control_initial_optimal_final.png}
}
\caption{Тестовая функция $u$, начальная $u_0$, конечная функция $u_{end}.$}
\label{control}
\end{figure}
...
{
	\label{fig2:exp1}
	\includegraphics[width=.51\linewidth]{first/cost_functional_dynamics.png}
}
\subfloat[Второй эксперимент]
{
	\label{fig2:exp2}
	\includegraphics[width=.51\linewidth]{second/cost_functional_dynamics.png}
}
