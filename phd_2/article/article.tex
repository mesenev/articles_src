...
\begin{document}
    ...
    Следовательно задача (\ref{w1}) однозначно разрешима.
    ...
    \end{center}
    Пусть функционал $J_\lambda (\theta, u)$ соответсвует условиям, представленным в гл.
    2.
    Согласно формуле~\eqref{OC2} градиент функционала качества равен
    \[
        J'_\lambda (u) = \lambda u - p_2.
    \]
    Здесь $u$ -- функция из граничного условия~\eqref{bc1}, $p_2$ -- соответствующая переменная сопряжённой
    системы~\eqref{L}--\eqref{L1}.
    ...
    $J(\theta_k, u_k) - J(\theta_{k+1}, u_{k+1}) < 10^{-12}$.
    Эксперименты показывают хорошее восстановление функции $u$ при $N > 10^{3}$.

    Приведём примеры расчётов для куба с ребром 1~см,
    ...
    Далее мы приведём два тестовых случая.
    Для первого теста определим параметры $r$ и $u$ из~\eqref{bc1} как:
    \begin{gather*}
        r = 0.8 cos(x) + 0.1,\\
        \hat u = y.
    \end{gather*}
    Из решения прямой задачи~\eqref{eq1}--\eqref{bc1} при заданных параметрах $r$ и $u$ расчитываем
    состоение $\theta$ и $\varphi$.
    Далее функцию $\theta_b$ из~\eqref{bc2} положим равным значению функции $\theta$ на $\Gamma$.
    Установим начальное значение $u_0 = 0.1$.
    Далее, применяя предложенный алгоритм находим приближенное решение задачи CP.
    Квадрат разницы тестового и найденного решения, а также динамика функционала качества представлена на рисунке 1.

    \begin{figure}[H]
        \centering
        \subfloat[ $(\hat u - u_{end}) ^ 2$]
        {
        \label{fig1:exp1}
        \includegraphics[width=.51\linewidth]{img/1_diff_end_control.png}
        }
        \subfloat[Динамика функционала качества]
        {
        \label{fig1:exp2}
        \includegraphics[width=.51\linewidth]{img/1_quality.png}
        }
    ...
    Отметим, что для данного теста мы не знаем оптимальное решение $u$.
    ...
    \begin{figure}[H]
        ...
        {
        \label{fig2:exp1}
        \includegraphics[width=.51\linewidth]{img/2_end_control.png}
        }
        \subfloat[Динамика функционала качества]
        {
        \label{fig2:exp2}
        \includegraphics[width=.51\linewidth]{img/2_quality.png}
        }
    ...
    В то же самое время, компонента излучения $\varphi$ изменяется весьма существенно.

    \begin{figure}[H]
        \centering
        \subfloat[$\theta$ на нулевой итерации]
        {
        \label{fig3:exp1}
        \includegraphics[width=.51\linewidth]{img/2_init_theta.png}
        }
        \subfloat[$\theta$ на последней итерации]
        {
        \label{fig3:exp2}
        \includegraphics[width=.51\linewidth]{img/2_end_theta.png}
        }
        \caption{Компонента температуры, соотвествующая управлению $u_0$ и $u_{100}$}
        \label{theta_dynamics}
    \end{figure}

    \begin{figure}[H]
        \centering
        \subfloat[Начальное значение $\varphi$]
        {
        \label{fig4:exp1}
        \includegraphics[width=.51\linewidth]{img/2_init_phi.png}
        }
        \subfloat[Конечное значение $\varphi$]
        {
        \label{fig4:exp2}
        \includegraphics[width=.51\linewidth]{img/2_end_phi.png}
        }
        \caption{Компонента излучения, соотвествующее управлению $u_0$ и $u_{100}$}
        \label{phi_dynamics}
    \end{figure}
