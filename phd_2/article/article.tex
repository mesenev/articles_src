...
\begin{document}
    ...
    Для первого и второго теста положим начальное значение $u_0 = 0.1$.
    ...
    Эффективность алгоритма, а также значение $u_0$ в первом и втором случаях представлены на рис. 1.

    \begin{figure}[H]
        \centering
        \subfloat[Разница тестового и найденного управления]
        {
        \label{fig1:exp1}
        \includegraphics[width=.51\linewidth]{img/diff_control_test_final.eps}
        }
        \subfloat[Динамика функционала качества]
        {
        \label{fig1:exp2}
        \includegraphics[width=.51\linewidth]{img/quality_1.eps}
        }
        \caption{Первый эксперимент}
        \label{img_control_1}
    \end{figure}

    Для второго эксперимента определим функции $\theta_b$, $q_b$ из~\eqref{bc2} следующим образом
    \[
        \theta_b = 0.1z + 0.3, \;
        q_b = \begin{cases}
                  -0.15, & \text{если } z = 0, \\
    ...
    В данном случае мы не знаем оптимальное решение $u$ задачи (1)-(2).
    ...
    \begin{figure}[H]
        ...
        {
        \label{fig2:exp1}
        \includegraphics[width=.51\linewidth]{img/final_control_2.eps}
        }
        \subfloat[Динамика функционала качества]
        {
        \label{fig2:exp2}
        \includegraphics[width=.51\linewidth]{img/quality_2.png}
        }
        \caption{Тестовая функция $u$, начальная $u_0$, найденная функция $u_{end}.$}
        \label{img_control_2}
    \end{figure}
    Отметим довольно незначительное изменение функционала качества (в десять раз).
    Это объясняется тем, что выбранное управление не всегда оказывает существенное влияние на температурное поле.
    В то же самое время, компонента излучения $\varphi$ изменяется по итерациям весьма существенно.

    \begin{figure}[H]
        \centering
        \subfloat[Начальное значение $\theta$]
        {
        \label{fig3:exp1}
        \includegraphics[width=.51\linewidth]{img/init_theta.png}
        }
        \subfloat[Конечное значение $\theta$]
        {
        \label{fig3:exp2}
        \includegraphics[width=.51\linewidth]{img/end_theta.png}
        }
    ...
    \begin{figure}[H]
        ...
        {
        \label{fig4:exp1}
        \includegraphics[width=.51\linewidth]{img/init_phi.png}
        }
        \subfloat[Конечное значение $\varphi$]
        {
        \label{fig4:exp2}
        \includegraphics[width=.51\linewidth]{img/end_phi.png}
        }
