...
r_default = Expression("0.8 * cos(x[1]*3.14/2) + 0.5*x[0] + 0.5 * sin(x[2]*3.14/2)", degree=3)
...
# print_3d_boundaries_on_cube(target_phi_n, folder=folder)
...
problem.find_optimal_control(iterations=1 * 10 ** 2, _lambda=10 * 7)
...
print_3d_boundaries_on_cube(problem.phi_n, name='theta_n_diff', folder=folder)
